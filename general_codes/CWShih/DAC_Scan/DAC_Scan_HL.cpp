...
void DAC_Scan_HL(TString set_folder_name, TString server_name, vector<int> FC_id)
{
    ...
    TString plot_folder_dire = Form("%s/data_analysis/%s/%s/%i_%s", mother_folder_directory, set_folder_name, server_name, FC_id[0], serverFC_toinfo_map[Form("%s_%i",server_name,FC_id[0])].Ladder.Data());
    ...
    {
        string file_name_short = string(file_name_vec[fid]); file_name_short = std::regex_replace(file_name_short, std::regex("^.*/"), "");
    ...
    ladder[0] -> Output_bin_plots(plot_folder_dire, set_folder_name);
