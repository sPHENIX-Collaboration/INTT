#include "Fun4All_Intt_SanityCheck.hh"

R__LOAD_LIBRARY(libINTTRawHitSanityCheck.so)
// R__LOAD_LIBRARY(libINTTBcoResolution.so)
// R__LOAD_LIBRARY(libdNdEtaINTT.so)
// R__LOAD_LIBRARY(libmbd.so)
// R__LOAD_LIBRARY(libEvtIDReporter.so)

void Fun4All_Intt_SanityCheck(
  int process_id = 0,
  int run_num = 54280,
  int nevents = 20,
  string output_directory = "/sphenix/tg/tg01/commissioning/INTT/work/cwshih/seflgendata/run_54280_CW_Mar252025_INTTQA",
  
  // note : for macro
  string input_directory = "/sphenix/lustre01/sphnxpro/production/run2auau/physics/ana464_nocdbtag_v001",
  ...
  int triggered_BcoDiff = 55,
  bool apply_hot_channel = true,
  bool clone_hit_remove_BCO = true,
  bool hitQA_check = true,
  bool check_clone_hit = true,

  bool HaveGL1 = true,
  bool Get_InttRawHit_ntuple = true
)
{
  ...
  for (int i = 0; i < 8; i++)
  {
    std::string intt_folder_direcotry = Form("DST_STREAMING_EVENT_INTT%d/run_00054200_00054300/dst", i);
    std::string intt_file_name = Form("DST_STREAMING_EVENT_INTT%d_run2auau_ana464_nocdbtag_v001-00054280-%s.root", i, job_index.c_str());
  ...
  // for (int i = 0; i < 1; i++)
  // {
  //   // std::string intt_folder_direcotry = Form("DST_STREAMING_EVENT_INTT%d/run_00054900_00055000/dst", i);
  //   // std::string intt_file_name = Form("DST_STREAMING_EVENT_INTT%d_run2auau_ana464_nocdbtag_v001-00054912-%s.root", i, job_index.c_str());

  //   input_managers.push_back(new Fun4AllDstInputManager(Form("DST_track_%d",i)));
  //   input_managers.back() -> Verbosity(2);
  //   input_managers.back() -> AddFile( input_directory + "/test_inttrawhit_54912_addfile_ana441_1M.root");
  //   se->registerInputManager(input_managers.back());
  ...
  INTTRawHitSanityCheck * IBR = new INTTRawHitSanityCheck (
