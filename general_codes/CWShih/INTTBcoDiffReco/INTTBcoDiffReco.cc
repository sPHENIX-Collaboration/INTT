...
// here and in INTTBcoDiffReco.h.
//
// INTTBcoDiffReco(const std::string &name = "INTTBcoDiffReco")
// everything is keyed to INTTBcoDiffReco, duplicate names do work but it makes
// e.g. finding culprits in logs difficult or getting a pointer to the module
// from the command line
//
// INTTBcoDiffReco::~INTTBcoDiffReco()
// this is called when the Fun4AllServer is deleted at the end of running. Be
// mindful what you delete - you do loose ownership of object you put on the node tree
//
// int INTTBcoDiffReco::Init(PHCompositeNode *topNode)
// This method is called when the module is registered with the Fun4AllServer. You
// can create historgrams here or put objects on the node tree but be aware that
// modules which haven't been registered yet did not put antyhing on the node tree
//
// int INTTBcoDiffReco::InitRun(PHCompositeNode *topNode)
...
//
// int INTTBcoDiffReco::process_event(PHCompositeNode *topNode)
...
//
// int INTTBcoDiffReco::ResetEvent(PHCompositeNode *topNode)
// If you have internal data structures (arrays, stl containers) which needs clearing
// after each event, this is the place to do that. The nodes under the DST node are cleared
// by the framework
//
// int INTTBcoDiffReco::EndRun(const int runnumber)
// This method is called at the end of a run when an event from a new run is
// encountered. Useful when analyzing multiple runs (raw data). Also called at
// the end of processing (before the End() method)
//
// int INTTBcoDiffReco::End(PHCompositeNode *topNode)
// This is called at the end of processing. It needs to be called by the macro
// by Fun4AllServer::End(), so do not forget this in your macro
//
// int INTTBcoDiffReco::Reset(PHCompositeNode *topNode)
// not really used - it is called before the dtor is called
//
// void INTTBcoDiffReco::Print(const std::string &what) const
// Called from the command line - useful to print information when you need it
//
//____________________________________________________________________________..

#include "INTTBcoDiffReco.h"
...
{
  std::cout << "INTTBcoDiffReco::INTTBcoDiffReco(const std::string &name) Calling ctor" << std::endl;

  if (clone_hit_remove_tag && clone_hit_remove_BCO_tag) {
    std::cout<<"INTTBcoDiffReco::INTTBcoDiffReco - clone_hit_remove and clone_hit_remove_BCO cannot be true at the same time"<<std::endl;
...
INTTBcoDiffReco::~INTTBcoDiffReco()
{
  std::cout << "INTTBcoDiffReco::~INTTBcoDiffReco() Calling dtor" << std::endl;
}

//____________________________________________________________________________..
int INTTBcoDiffReco::Init(PHCompositeNode *topNode)
{
  std::cout << "INTTBcoDiffReco::Init(PHCompositeNode *topNode) Initializing" << std::endl;
...
int INTTBcoDiffReco::PrepareOutFile()
{
...
int INTTBcoDiffReco::PrepareHotChannel()
{
  hot_file_in = TFile::Open(hotchannel_full_directory.c_str());

  if (!hot_file_in)
  {
    std::cout << "INTTBcoDiffReco::PrepareHotChannel - hot channel file not found" << std::endl;
    gSystem->Exit(1);
    exit(1);
  }
...
int INTTBcoDiffReco::InitRun(PHCompositeNode *topNode)
{
  std::cout << "INTTBcoDiffReco::InitRun(PHCompositeNode *topNode) Initializing for Run XXX" << std::endl;
  return Fun4AllReturnCodes::EVENT_OK;
}

std::vector<int> INTTBcoDiffReco::prepare_trigger_vec(long long trigger_input)
{
...
std::map<int,int> INTTBcoDiffReco::prepare_trigger_map(std::vector<int> trigger_vec_input)
{
...
int INTTBcoDiffReco::PrepareGL1(PHCompositeNode *topNode)
{
  gl1 = nullptr;
  out_GL1_bco = -999;
  ...
  if (!gl1)
  {
    std::cout << "INTTBcoDiffReco::process_event - GL1RAWHIT node not found" << std::endl;
  ...
  {
    std::cout << "INTTBcoDiffReco::PrepareGL1 - live trigger is negative, "<< live_trigger_decimal << std::endl;
    return Fun4AllReturnCodes::EVENT_OK;
  }
...
std::vector<int> INTTBcoDiffReco::GetHistBinEntry(TH1F * hist)
{
  std::vector<int> output_vec; output_vec.clear();
...
int INTTBcoDiffReco::PrepareINTT(PHCompositeNode *topNode)
{
  ...
  if (out_total_nHit == 0) {
    out_INTT_bcofull = -999;
    std::cout << "eID: "<< eID_count <<" INTTBcoDiffReco::PrepareINTT - no INTT hit found" << std::endl;
    return Fun4AllReturnCodes::EVENT_OK;
  }
  ...
  for (unsigned int i = 0; i < inttcont->get_nhits(); i++)
  {
    ...
    {
      std::cout << "eID: "<< eID_count <<" INTTBcoDiffReco::PrepareINTT - server out of range, this server is : "<< server << std::endl;
      continue;
    }

    if (hitQA_tag && (felix_ch < 0 || felix_ch > 13)) // note : felix channel ID 0 - 13
    {
      std::cout << "eID: "<< eID_count <<" INTTBcoDiffReco::PrepareINTT - felix channel out of range, this channel is : "<< felix_ch << std::endl;
      continue;
    }

    if (hitQA_tag && (chip < 0 || chip > 25)) // note : chip ID 0 - 25
    {
      std::cout << "eID: "<< eID_count <<" INTTBcoDiffReco::PrepareINTT - chip out of range, this chip is : "<< chip << std::endl;
      continue;
    }

    if (hitQA_tag && (channel < 0 || channel > 127)) // note : channel ID 0 - 127
    {
      std::cout << "eID: "<< eID_count <<" INTTBcoDiffReco::PrepareINTT - channel out of range, this channel is : "<< channel << std::endl;
      continue;
    }

    if (hitQA_tag && (adc < 0 || adc > 7)) // note : adc ID 0 - 7
    {
      std::cout << "eID: "<< eID_count <<" INTTBcoDiffReco::PrepareINTT - adc out of range, this adc is : "<< adc << std::endl;
      continue;
    }
    
    if (hitQA_tag && (bco < 0 || bco > 127)) // note : bco ID 0 - 127
    {
      std::cout << "eID: "<< eID_count <<" INTTBcoDiffReco::PrepareINTT - bco out of range, this bco is : "<< bco << std::endl;
      continue;
    }
  ...
  for (const auto& pair : evt_inttHits_map)
  {
    ...
    {
      std::cout << "eID: "<< eID_count <<" INTTBcoDiffReco::PrepareINTT - bco_diff out of range, this bco_diff is : "<< bco_diff<<" hitbco : "<<bco<<" bco_full : "<<out_INTT_bcofull<< std::endl;
...
int INTTBcoDiffReco::PrepareHistInfo(TH1F * hist)
{
...
int INTTBcoDiffReco::PrepareCloneHitInfo()
{
...
int INTTBcoDiffReco::process_event(PHCompositeNode *topNode)
{
  // std::cout << "INTTBcoDiffReco::process_event(PHCompositeNode *topNode) Processing Event" << std::endl;
  ...
  if (evt_felix_BcoDiffHist_vec[0]->GetEntries() != 0) {
    std::cout << "eID: "<< eID_count << " INTTBcoDiffReco::process_event - evt_felix_BcoDiffHist_vec[0] is not empty" << std::endl;
    gSystem->Exit(1);
    exit(1);
  }
...
int INTTBcoDiffReco::ResetEvent(PHCompositeNode *topNode)
{
  // std::cout << "INTTBcoDiffReco::ResetEvent(PHCompositeNode *topNode) Resetting internal structures, prepare for next event" << std::endl;
  return Fun4AllReturnCodes::EVENT_OK;
}
...
int INTTBcoDiffReco::EndRun(const int runnumber)
{
  ...
  std::cout << "INTTBcoDiffReco::EndRun(const int runnumber) Ending Run for Run " << runnumber << std::endl;
  return Fun4AllReturnCodes::EVENT_OK;
}
...
int INTTBcoDiffReco::End(PHCompositeNode *topNode)
{
  std::cout << "INTTBcoDiffReco::End(PHCompositeNode *topNode) This is the End..." << std::endl;
  return Fun4AllReturnCodes::EVENT_OK;
}
...
int INTTBcoDiffReco::Reset(PHCompositeNode *topNode)
{
 std::cout << "INTTBcoDiffReco::Reset(PHCompositeNode *topNode) being Reset" << std::endl;
  return Fun4AllReturnCodes::EVENT_OK;
}
...
void INTTBcoDiffReco::Print(const std::string &what) const
{
  std::cout << "INTTBcoDiffReco::Print(const std::string &what) const Printing info for " << what << std::endl;
}
