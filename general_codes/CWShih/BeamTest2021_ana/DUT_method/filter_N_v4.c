...
//
// New feature (v4) 2022/01/05 : not only study the hits in the first group. the hits in the rest group are also considered. 
//                               this is not suitable for the bco phase study. just to increase the statistic. 
//                               the performance is still uncertain now, we haven't really understood what are those hits. 
//
...
void filter_N_v4 (TString file_name)
{
	//===================================option===========================================
	// TString file_name = "BeamData_20211210-2043_0";
	TString path_in = "/data4/chengwei/Geant4/INTT_simulation/G4/for_CW/data/run52_MC_2/";
	//===================================option===========================================
	...
	int module_l1 =6;
	int module_l2 =5;
	int module_l3 =100; //don't need to see this 
	//I don't understand....
	...
	int vec_element;
	int DSE_element = 0; // the default is 0 that means "not a double saving event"
	...
	int clock_counter=0;
	int clock_element_counter=0;
	vector <int> clock_element; clock_element.clear();
	int bco_safer, bco_full_safer;
	...
	TFile *file_output = new TFile(path_in+file_name+"_filter_all_int_1000.root", "RECREATE");
	// file_output->cd();
	...
	// tree_output3->Branch("clock_counter"    ,&clock_counter); //how many bco clock (group) in one event
	// tree_output3->Branch("clock_ele"        ,&clock_element); //how many hits in each bco clock (group) of an event.
	//======================preparation of recreate================================
	...
	int DSE_classification_counter = 0; // we have 2 different DSE case, 1. same adc, 2. different adc. we want to classify them 
	...
	for (int i=0; i<nEvent_both; i++) // # of total events
	{
		...
		// ================== to study the # of BCO group in one event (one vector) ==================
		// ================== no cut ==================
		// if (lenvec > 0)
		// {
		// 	bco_safer = vec_bco->at(0);
		// 	bco_full_safer = vec_bco_full->at(0);
		// 	clock_counter = 1;
		// }
		// else 
		// {
		...
		// {
		// 	if (vec_bco->at(j) == bco_safer && vec_bco_full->at(j) == bco_full_safer)
		// 	{
		// 		clock_element_counter+=1;
		// 	}
		// 	else 
		// 	{
		// 		clock_counter+=1;
		// 		bco_safer = vec_bco->at(j);
		// 		bco_full_safer = vec_bco_full->at(j);
		// 		clock_element.push_back(clock_element_counter);
		// 		clock_element_counter = 1;
		// 	}
		// }
		// if (lenvec > 0)
		...
		if (lenvec > 0)
		{
			first_bco      = vec_bco->at(0);
			first_bco_full = vec_bco_full->at(0);	
		}
		for (int j=0; j<lenvec; j++)
		{
			// the filter only works on the first BCO group of one trigger event. 
			if (vec_bco->at(j) == first_bco && vec_bco_full->at(j) == first_bco_full)
			{
				//the first selection is module ID
				if (vec_module->at(j) == module_l0)
				{
					if (vec_adc_filter [0][vec_chip_id->at(j)-1][vec_chan_id->at(j)] != -1 )
					{
						if (vec_adc_filter [0][vec_chip_id->at(j)-1][vec_chan_id->at(j)] != vec_adc->at(j))
						{
							DSE_classification_counter +=1;
						}
				...
				{
					if (vec_adc_filter [1][vec_chip_id->at(j)-1][vec_chan_id->at(j)] != -1 )
					{
						if (vec_adc_filter [1][vec_chip_id->at(j)-1][vec_chan_id->at(j)] != vec_adc->at(j))
						{
							DSE_classification_counter +=1;
						}
						DSE_element = 1;
				...
				{
					if (vec_adc_filter [2][vec_chip_id->at(j)-1][vec_chan_id->at(j)] != -1 )
					{
						if (vec_adc_filter [2][vec_chip_id->at(j)-1][vec_chan_id->at(j)] != vec_adc->at(j))
						{
							DSE_classification_counter +=1;
						}
						DSE_element = 1;
				...
				{
					if (vec_adc_filter [3][vec_chip_id->at(j)-1][vec_chan_id->at(j)] != -1 )
					{
						if (vec_adc_filter [3][vec_chip_id->at(j)-1][vec_chan_id->at(j)] != vec_adc->at(j))
						{
							DSE_classification_counter +=1;
						}
						DSE_element = 1;
			...
			{
				first_bco = vec_bco->at(j);
				first_bco_full = vec_bco_full->at(j);
				...
				if (DSE_element == 1 && DSE_classification_counter > 0)
				{
					DSE_element = 2;
				}
				...
				if (sub_eID > 1000)
				{
					cout<<"problem report 0 : sub_eID > 1000"<<endl;
				}
		...
		if (sub_eID > 1000)
		{
			cout<<"problem report 1 : sub_eID > 1000"<<endl;
		}
		tree_output3->Fill();
