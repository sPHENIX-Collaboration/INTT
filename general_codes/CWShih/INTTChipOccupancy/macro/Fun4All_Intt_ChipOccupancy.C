#include "Fun4All_Intt_ChipOccupancy.hh"

R__LOAD_LIBRARY(libINTTChipOccupancy.so)
// R__LOAD_LIBRARY(libINTTHitMap.so)
// R__LOAD_LIBRARY(libdNdEtaINTT.so)
// R__LOAD_LIBRARY(libmbd.so)
// R__LOAD_LIBRARY(libEvtIDReporter.so)

void Fun4All_Intt_ChipOccupancy(
  int process_id = 0,
  int run_num = 54280,
  int nevents = -1,
...
)
{
  ...
  INTTChipOccupancy * ICO = new INTTChipOccupancy (
    "INTTChipOccupancy",
    process_id,
    run_num,
    output_directory,
