...
R__LOAD_LIBRARY( libInttClusterAnalyzer.so ) // for InttVertexXY
	    ...
	    )
{
  ...
  InttXYVertexFinder* intt_vtx = new InttXYVertexFinder( "intt_vertex_finder" );
  //intt_vtx->Verbosity( 100 );
  intt_vtx->SetPeriod( nEvents - 1 );
  se->registerSubsystem( intt_vtx );
  
  //////////////////////////
  // Your analysis module // 
  //////////////////////////
  InttVertexXY* vertex = new InttVertexXY( "vertex" );
  vertex->SetRunNum( run );
