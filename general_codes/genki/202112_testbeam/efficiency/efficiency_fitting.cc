...
*/
int efficiency_fitting( int run_num = 89, string alignment_dir = "results/ELPH/alignment/fitting/ana/normal/", bool is_MC = false  )
{
  ...
  const string kOutput_dir = kOutput_root_dir + "efficiency/fitting/" ;
  ...
  if( !is_MC )
    {
      //string alignment_result = Replace( output, "efficiency", "alignment" );
      string alignment_result = alignment_dir + "run" + to_string( run_num ) + ".root";
      INTTEvent::SetAlignmentMap( alignment_result );
    }
	  ...
	  for( int k = hot_chips[j]-2; k<hot_chips[j]+2+1; k++ )
	      ...
	      if( !is_MC )
		{
		  ...
		  if( j == 0 )
		    ...
		    event->SetAlignmentOffset( 0, -1.15105 );
		  else if( j == 1 )
		    event->SetAlignmentOffset( 1, 2.30759 );
		  else if( j == 2 )
		    event->SetAlignmentOffset( 2, -1.15010 );
		  */
		  event->Print();
		}
	      else
		{
		  event->SetAlignmentOffset( j, 0.0 );
		}
		  ...
		  if( index != -1 )
		    {
		      ...
		      chan_id_out[m] = (*chan_id)[index] - event->GetAlignmentOffset( INTTEvent::Module2Ladder( module_out[m]) );
