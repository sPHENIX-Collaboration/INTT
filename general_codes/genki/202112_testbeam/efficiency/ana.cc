...
void CalcEfficiency0D( TCanvas* c, TTree* tr, int ladder_num, string cut_arg, vector < pair < TObject*, string> >& obj_saved )
{
  ...
  for( int i=first_chip; i<last_chip; i++ )
	...
	cut += " && " + cut_arg;
      ...
      string hist_name_base = string("ladder") + to_string(ladder_num)
	+ "_chip" + to_string( i );
      
      string hist_name_numerator = "hist_numerator_" + hist_name_base;
      TH1D* hist_numerator = GetHist( tr, hist_name_numerator, expression, cut, bin, xmin, xmax );
      gPad->SetLogy( true );
      hist_numerator->Draw( "HISTE" );
      DrawStats( hist_numerator, 0.6, 0.6, 0.9, 0.9 );
      c->Print( c->GetName() );

      obj_saved.push_back( pair < TObject*, string >(hist_numerator, hist_numerator->GetName()) );
	...
	cut_deno += " && ";
      ...
      string hist_name_denominator = "hist_denominator_" + hist_name_base;
      TH1D* hist_denominator = GetHist( tr, hist_name_denominator, expression, cut_deno, bin, xmin, xmax );
      gPad->SetLogy( true );
      ...
      //TF1* fit = new TF1( "f", formula.c_str(), xmin, xmax ); // the range of the fitting function shouldn't be too large, otherwise, TF1::Integral is failed.
      TF1* fit = new TF1( "f", formula.c_str(), -100, 100 ); // range -100 - +100ch seems to be fine. 
      fit->SetNpx( 1e4 );
      // 1st fitting -- only primary Gaussian ////////////////////////
      //fit->SetParLimits(1, hist_denominator->GetMean() - 10, hist_denominator->GetMean() + 10 );
      fit->SetParLimits(0, GetMaxBinContent( hist_denominator ) * 0.1, GetMaxBinContent( hist_denominator ) * 10 );
      fit->SetParLimits(1, hist_denominator->GetMean() - 5, hist_denominator->GetMean() + 5 );
      fit->SetParLimits(2, 0, 10 );
      fit->SetParLimits(5, 3, 1e2 );
      ...
      hist_denominator->Fit( fit, "L", "", hist_denominator->GetMean() - 3, hist_denominator->GetMean() + 3 );
      ...
      for( int i=0; i<6; i++ ) // for the primary gaussian
	{
      ...
      fit->SetParLimits( 3, 1, GetMaxBinContent( hist_denominator ) * 0.1 );
	...
	fit->ReleaseParameter( i );
      ...
      fit->SetParLimits( 0, fit->GetParameter(0) * 0.8 , fit->GetParameter(0) * 1.2 );
      fit->SetParLimits( 1, fit->GetParameter(1) - 3 , fit->GetParameter(1) + 3 );
      fit->SetParLimits( 2, 0, fit->GetParameter(2) * 1.5 );
      //fit->SetParLimits( 3, fit->GetParameter(3) * 0.8 , fit->GetParameter(3) * 1.2 );
      ...
      //fit->SetParLimits( 4, fit->GetParameter(2), fit->GetParameter(4) * 1.5 );
      fit->SetParLimits( 3, 0, fit->GetParameter(0) * 0.1  );
      ...
      obj_saved.push_back( pair < TObject*, string >(hist_denominator, hist_denominator->GetName() ) );
      ...
      for( int j=3; j<5; j++ )
	{
	  ...
	  double double_gauss_3sigma_min, double_gauss_3sigma_max;
	  if( j==4 )
	    {
	      Get3SigmaRange( fit, double_gauss_3sigma_min, double_gauss_3sigma_max );
	      
	      cut_region = cut_deno + " && "
		+ to_string( double_gauss_3sigma_min ) + "<diff_chan"
		+ " && " + "diff_chan<" + to_string( double_gauss_3sigma_max );
	    }
	  ...
	  if( j != 4 )
	    ...
	    box->DrawBox( double_gauss_3sigma_min, 0, double_gauss_3sigma_max, height * 2 );
	  ...
	  if( j == 4 )
	    {
	      comment.str( "" );
	      comment << "* " << setprecision(3) << double_gauss_3sigma_min << "ch - "
		      << setprecision(3) << double_gauss_3sigma_max << "ch"
		      << "(99.73% CI)";
	    }
...
int ana( int run_num = 89, string type = "simplest",
	 ...
	 string cut_arg = "", bool is_MC = false )
{
  ...
  string output_root_dir = "results/ELPH/efficiency/" + type + "/";
