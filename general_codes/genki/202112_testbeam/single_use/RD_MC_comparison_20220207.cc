...
int RD_MC_comparison_20220207()
{
  ...
  string data_MC_smear_new = "repo/build/debug_smeared/ED_convert.root";
  auto hist_MC_smear_new = GetHist( data_MC_smear_new, 1, "MC_updated" );
  HistSetting( hist_MC_smear_new, kGreen+2  );
  hist_MC_smear_new->SetMarkerStyle( 1 );

  string data_MC_smear_shifted = "repo/build/ED_y-3mm_convert.root";
  auto hist_MC_smear_shifted = GetHist( data_MC_smear_shifted, 1, "MC_updated_shifted" );
  HistSetting( hist_MC_smear_shifted, kOrange + 1  );
  hist_MC_smear_shifted->SetMarkerStyle( 1 );

  string output = "results/ELPH/single_use/RD_MC_comaprison_20220207.pdf";
  TCanvas* c = GetCanvas( output, "ratio", false );
  MultiHist* mh = new MultiHist( "Channel distribution", "Channel distribution;Channel;Entries" );
  mh->Add( hist_RD );
  mh->Add( hist_MC_mono );
  mh->Add( hist_MC_smear );
  mh->Add( hist_MC_smear_new );
  mh->Add( hist_MC_smear_shifted );
  mh->NormalizeHist();
  
  c->cd(1);
  mh->Draw( "HISTE" );

  hist_RD->Scale( 1.0 / hist_RD->Integral() );
  hist_MC_smear_shifted->Scale( 1.0 / hist_MC_smear_shifted->Integral() );
  MultiHist* mh_ratio = new MultiHist( "ratio", "ratio", hist_MC_smear_shifted, hist_RD, false );
