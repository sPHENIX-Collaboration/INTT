...
/*!
  @fn int check_chip_prototypeMaximam6_with_cut
  @brief 
...
*/
int check_chip_prototypeMaximam6_with_cut
(
 ...
 )
{
  ...
  string root_file = MakeTree(fname, usemod, mode, cut, decoded_out);
  //return 0;

  // If there was no error in MakeTree, draw some plots!
  if (root_file != "")
    DrawPlots(root_file, usemod, mode);
...
string MakeTree(string fname, int usemod, string mode, string cut, bool decoded_output)
{
  ...
  output = output.substr(0, output.find_last_of(".dat") - 3) + "_with_cut.root"; // it means, for example, nwu_fphx_20200101_000000_0.dat -> nwu_fphx_20200101_000000_0 + .root
  ...
  for (int index = 0; index < size / sizeof(int); bufcnt++) { // note: value of index is changed not only here
      ...
      if( mode != "calib" && mode != "external" )
	{
	  ...
	  if( fname.find( "nwu_fphx_raw_20201113-0137_0.dat" ) != string::npos || 
	      ...
	      for (int i=0; i < camac_ADC_num; i++)
		{
	      ...
	      //cout << "CANAC TDC num: " << camac_TDC_num << endl;
	...
	MakeCluster( adcs     , adc_voltages, ampls   , chip_ids, fpga_ids,
      ...
      if( find( chip_ids.begin(), chip_ids.end(), 3 ) != chip_ids.end() )
	{
	  cout << "----------------------------------------" << endl;
	  for( const auto& it : chip_ids )
	    cout << it << endl;
	  tree_both->Fill();
	}
...
void ShowMessage(){
  ...
  ShowLine( width, " check_chip_prototypeMaximam6.c" );
  ShowLine( width, " It's for data with the external trigger system by CAMAC" );
  ShowLine(width, "");
  ShowLine( width, " Usage: .x check_chip_prototypeMaximam6.c( file_name, module_num, mode, cut)" );
