void check_xyzsim_evt(int ievent=0){
  gStyle->SetOptStat(0);

  TFile *f = TFile::Open("AnaTutorial_SimZF.root");
  ...
  c1->cd(1);
  ntp_clus->Draw("y:x>>hxy",                        Form("size<5&&adc<10&&evt==%d", ievent));
  c1->cd(2);                                                    
  ntp_clus->Draw("(y<0?-1:1)*sqrt(y*y+x*x):z>>hrz", Form("size<5&&adc<10&&evt==%d", ievent));
  c1->cd(3);                                                    
  ntp_clus->Draw("y:x:z>>hxyz", Form("size<5&&adc<10&&evt==%d", ievent));
}
