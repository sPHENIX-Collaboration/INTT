...
void MakeCluster( int index_judged,
		 vector < int >& adcs,
		 vector < int >& ampls,
		 vector < int >& chip_ids,
		 vector < int >& fpga_ids,
...
void ShowMessage();
/*!
  @fn int check_chip_prototypeMaximam6
  @brief 
...
string MakeTree(string fname, int usemod, string mode, string cut, bool decoded_output)
{
  ...
  vector < int >  adcs, ampls, chip_ids, fpga_ids, modules, chan_ids, fem_ids, bcos,  bco_fulls, events;
  ...
  for (int index = 0; index < size / sizeof(int); bufcnt++) { // note: value of index is changed not only here
	...
	is_INTT = true;
      ...
      if( mode == "camac_clustering" )
				MakeCluster( adcs     , ampls   , chip_ids, fpga_ids,
		 ...
		 vector < int >& events)
{
	...
	INTTHit* hits[100];		//vector < INTTHit* > hits;
