...
void InttBCOFinder(int i = 20869, bool isMerged = true,std::string runtype_ = "beam", bool ADC0cut_ = false)
{
  ADC0cut = ADC0cut_;
  ...
  merged_inputFile = "/sphenix/tg/tg01/commissioning/INTT/work/jaein/DataProduction/combined-00020869.root";
...
void bcofull_bco_merged(int runnumber)
{
  ...
  TTree *tree = (TTree *)f->Get("intt_tree");
  vector<short> *bco=nullptr;
  vector<short> *chn=nullptr;
  vector<short> *chp=nullptr;
  vector<short> *fee=nullptr;
  vector<short> *pid=nullptr;
  vector<short> *adc=nullptr;
  Long64_t gtm;
  ...
  for (int j = 0; j < loop; j++)
  {
    ...
    for (size_t k = 0; k < bco->size(); k++)
    {
      ...
      if(felixnumber < 0 || felixnumber>7)
      {
        std::cout<<"BUG"<<std::endl;
      }
      uint64_t lo_bco_full = gtm;
