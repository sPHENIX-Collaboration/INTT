...
int init_params(struct params_s&, int const&, int const&);
...
int main(int argc, char** argv) {
	...
	if(init_params(pars, run_num, sig_cut)) return 1;
	...
	for(Long64_t n = 0; n < pars.tree->GetEntriesFast(); ++n) {
		...
		pars.cdbttree->SetIntValue(size, "chip", pars.raw.chip-1);
		pars.cdbttree->SetIntValue(size, "channel", pars.raw.channel);
		pars.cdbttree->SetIntValue(size, "flag", pars.state);
		++size;
...
int init_params(
	...
	//std::string STATE_FORMAT = "beam_intt_classified-%08d.root";
	//std::string CDB_FORMAT = "intt_cdb-%08d.root";
	//std::string TREENAME = "status_tree";
	std::cout<<run_num<<std::endl;
	std::cout<<sig_cut<<std::endl;
	std::string STATE_FORMAT = "/sphenix/tg/tg01/commissioning/INTT/QA/hotdeadmap/rootfile/2023/InttHotDeadMap_%d_%d.root";
	std::string CDB_FORMAT = "/sphenix/tg/tg01/commissioning/INTT/QA/hotdeadmap/CDB/2023/InttHotDeadMap_cdb-%08d_%d.root";
	std::string TREENAME = "tree";
	std::string STATE_BRANCHNAME = "flag";
	std::map<std::string, int*> BRANCHNAMES = {
		{"felix", &(pars.raw.felix_server)},
		{"module", &(pars.raw.felix_channel)},
		{"chip_id", &(pars.raw.chip)},
		{"chan", &(pars.raw.channel)},
	};
	...
	snprintf(buff, sizeof(buff), STATE_FORMAT.c_str(), run_num,sig_cut);
	FILENAME = buff;
	...
	snprintf(buff, sizeof(buff), CDB_FORMAT.c_str(), run_num,sig_cut);
	FILENAME = buff;
