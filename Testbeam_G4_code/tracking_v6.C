...
// 7. alignment                    (optional), 4 layers only now
// 
// 8. 3 or 4 layers 
		...
		for (int i1=0; i1<100; i1++ )
		{
			...
			else 
			{
				...
				if (ploting == true && event_id%info_sampling==0 )
				{
					...
					hit4_fit_latex->DrawLatex(0.12, 0.750, Form("#chi^{2}/NDF : %.4f", hit_fit_rchi_new[0]));
					hit4_fit_latex->DrawLatex(0.12, 0.720, Form("#chi^{2}/NDF : %.4f", hit_fit_rchi_new[1]));
					hit4_fit_latex->DrawLatex(0.12, 0.690, Form("#chi^{2}/NDF : %.4f", hit_fit_rchi_new[2]));
					hit4_fit_latex->DrawLatex(0.12, 0.660, Form("#chi^{2}/NDF : %.4f", hit_fit_rchi_new[3]));
...
void tracking_v6 () // by analyzing the tree_both
{
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
	const int number_of_layer=4; // number of layers of the INTT ladders in the Testbeam experiment. 
 	bool raw_plot_print = true; //whether you want to draw the event display or not 
	...
	bool alignment_on = false; // the alignment setting, true -> we do the alignment study. It takes time 
	...
	if (alignment_on == true) // the folder for all alignment plots
	{
	...
	TF1 *hit3_fit = new TF1("hit3_fit","pol1",-1,10);
	...
	TF1 *hit4_fit = new TF1("hit4_fit","pol1",-1,10);
				...
				for (int l1=0; l1<data_all_MC_EW[ int(hit3_layer[0]) ].size(); l1++)
				{
					for (int l2=0; l2<data_all_MC_EW[ int(hit3_layer[1]) ].size(); l2++)
					{
						for (int l3=0; l3<data_all_MC_EW[ int (hit3_layer[2]) ].size(); l3++)
						{
							...
							{
								hit3_grr ->SetMarkerStyle(20);
								hit3_grr ->SetMarkerSize(2);
								hit3_grr ->GetXaxis()->SetLimits(-1,5);
								hit3_grr ->GetXaxis()->SetTitle("Layer ID");
								hit3_grr ->GetYaxis()->SetRangeUser(-10,10);
								hit3_grr ->GetYaxis()->SetTitle("Y position (Unit : mm)");
								c1->cd();
								hit3_grr->Draw("apl");
							...
							if (  ( hit3_fit->GetChisquare()/double(hit3_fit->GetNDF()) ) < hit3_best_fit_picker)
							{
								...
								hit3_best_fit_picker_layer[3] = ( hit3_fit_Y[0] - (hit3_fit->GetParameter(1)* hit3_layer[0] + hit3_fit->GetParameter(0)) );//deviation from fit firstlayer
			...
			// then we check its residuals. If one of the residuals is larger than the setting cut, the further analysis is applied
		 	else if (most_hit_info[1]==4)//4 layer, all layers have hits ->4 
		 	{
					...
					for (int l2=0; l2<data_all_MC_EW[ 1 ].size(); l2++)
					{
						for (int l3=0; l3<data_all_MC_EW[ 2 ].size(); l3++)
						{
							for (int l4=0; l4<data_all_MC_EW[ 3 ].size(); l4++)	
							{
								...
								TGraph * hit4_grr = new TGraph (4,hit4_layer,hit4_fit_Y);
								hit4_grr ->Fit("hit4_fit","NQ");
								...
								if (  ( hit4_fit->GetChisquare()/double(hit4_fit->GetNDF()) ) < hit4_best_fit_picker)
								{
									hit4_best_fit_picker =( hit4_fit->GetChisquare()/double(hit4_fit->GetNDF()) );
									...
									hit4_best_fit_picker_layer[4] = ( hit4_fit_Y[0] - (hit4_fit->GetParameter(1)* double(hit4_layer[0]) + hit4_fit->GetParameter(0)) );//deviation from fit firstlayer
									hit4_best_fit_picker_layer[5] = ( hit4_fit_Y[1] - (hit4_fit->GetParameter(1)* double(hit4_layer[1]) + hit4_fit->GetParameter(0)) );//deviation from fit secondlayer
									...
									hit4_best_fit_picker_layer[13] = hit4_fit->GetParameter(0);
				...
				if (fabs(hit4_best_fit_picker_layer[4])>noise_hit_distance || fabs(hit4_best_fit_picker_layer[5])>noise_hit_distance || fabs(hit4_best_fit_picker_layer[6])>noise_hit_distance || fabs(hit4_best_fit_picker_layer[7])>noise_hit_distance)
				{
					...
					TGraph * hit4_grr = new TGraph (3,&hit4_secondfit_X[0],&hit4_secondfit_Y[0]);
					hit4_grr ->SetTitle( Form("4 hit PLC : %d, eID : %d, chip : %d, chan :%.3f, %.3f, %.3f,%.3f",pass_l1_counting,eID_array[i],most_hit_info[0],hit4_best_fit_picker_layer[0],hit4_best_fit_picker_layer[1],hit4_best_fit_picker_layer[2],hit4_best_fit_picker_layer[3]) );
					hit4_grr ->SetMarkerStyle(20);
					hit4_grr ->SetMarkerSize(2);
					hit4_grr ->GetXaxis()->SetLimits(-1,5);
					hit4_grr ->GetXaxis()->SetTitle("Layer ID");
					...
					TGraph * hit4_grr_all = new TGraph (4,hit4_layer,hit4_best_fit_picker_layer);
					hit4_grr_all ->SetMarkerStyle(20);
					hit4_grr_all ->SetMarkerSize(2);
					...
					//procedure 3
					hit4_second_residual[0] = hit4_secondfit_Y[0] - (hit4_fit->GetParameter(1)* double(hit4_secondfit_X[0]) + hit4_fit->GetParameter(0));
	...
	else if (number_of_layer==3)
	{
		...
		cout<<"layer 3 final counting : "<<N_HHH<<" "<<N_LHH<<" "<<N_HLH<<" "<<N_HHL<<endl;
